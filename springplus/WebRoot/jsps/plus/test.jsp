  ...
  <head>
	...
	<script type="text/javascript">
		...
		function search(page){
			...
			$post("select/test_select_by_where/"+page,{guanjianci:guanjianci,maxrow:mygrid.maxrow},function(data){
				mygrid.clear();
				var list=data["list"];
				for(var i=0;i<list.length;i++){
					...
					thedata["btn"]='<span data="'+thedata["id"]+'" onclick="addOrUpdate(this)" class="gridlike">修改</span>';
					thedata["btn"]=thedata["btn"]+'<span data="'+thedata["id"]+'" onclick="del(this)" class="gridlike">删除</span>';
		...
		function downloadexcel(){
			var guanjianci=$.trim($("#guanjianci").val());
			//test_select_by_where  这个语句必须有 order by 主键
			post("select/test_select_by_where/excel",{guanjianci:guanjianci,excelname:"测试excel报表"},function(data){
				downloadfile(data);
			},"text");
		}
		//增加或修改
		function addOrUpdate(element){
			var thedata=new Object();
			thedata["id"]=trim($(element).attr("data"));
			openurl("plus/test_add_update",thedata);
		}
		//删除
		function del(element){
			if(confirm("您确认要删除此数据吗?")){
				var id = $(element).attr("data");
				post("delete/TestModel",{id:id},function(data){
					if(data==1){//返回影响的行数
						alert("修改成功");
						openurl("plus/test");
					}else{
  ...
  <body>
	  <div class="ub ub-ver" style="height: 100%;">
	    <div class="ub" style="height: 40px;background-image: url('resource/imgs/head-ico1.png');background-size:40px 40px;">
	    	...
	    	<div class="title" onclick="addOrUpdate(this)" style="width: 80px;margin-right: 10px;text-align: center;line-height: 40px;font-size: 15px;cursor: pointer;" title="新增用户">新增用户</div>
