...
<html>
  <head>
	...
	<script type="text/javascript">
		...
		$(function(){
			if(updateId != ""){//如果是修改
				$post("select/TestModel/byid",{id:updateId},function(thedata){
					//填充数据
					fillForm("modeldiv",thedata);
				},"json");
			}
		});
		function addOrUpdate(element){//新增/修改
			//验证非空+收集数据
			...
			if(formdata !=null){
				if($(element).text()=="确定新增"){
					...
					post("select/test_check_username_isused/one",{username:formdata["username"]},function(data){
						if(data==0){
							post("insert/TestModel",formdata,function(result){
								if(result==1){
									alert("用户添加成功");
									openurl("plus/test");
								}
					...
					$post("update/TestModel",formdata,function(data2){
						if(data2==1){
							alert("修改成功");
							openurl("plus/test");
						}else{
	 ...
	 <div id="modeldiv" class="scrollbar" style="height: 100%;width: 100%;overflow: auto;">
	 	<div class="ub" style="height: 40px;background-image: url('resource/imgs/head-ico1.png');background-size:40px 40px;">
	    	<div class="ub ub-f1" style="height: 100%;line-height: 40px;padding-left: 10px;font-size: 13px;">
	    		...
	    		<div style="padding-left:17px;height: 100%;background-image: url(resource/imgs/head-ico.png);margin-left: 10px;margin-right: 15px;"></div>
	    		<div><%= StringUtil.hashText(updateId)?"修改":"新增" %></div>
	    		<div style="padding-left:17px;height: 100%;background-image: url(resource/imgs/head-ico.png);margin-left: 10px;margin-right: 15px;"></div>
	    	</div>
	    	<div class="title" onclick="addOrUpdate(this)" style="width: 80px;margin-right: 10px;text-align: center;line-height: 40px;font-size: 15px;cursor: pointer;" title="确定">确定<%= StringUtil.hashText(updateId)?"修改":"新增" %></div>
	    </div>
	 	<div>
