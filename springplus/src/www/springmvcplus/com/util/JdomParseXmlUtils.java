...
public class JdomParseXmlUtils {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static WeiXinParameter getWXPayResult(String xml){
		WeiXinParameter wXPayResult = new WeiXinParameter();
		try { 
			StringReader read = new StringReader(xml);
			// 创建新的输入源SAX 解析器将使用 InputSource 对象来确定如何读取 XML 输入
			InputSource source = new InputSource(read);
			...
			if(list!=null&&list.size()>0){
				for (Element element : list) {
					...
					if("total_fee".equals(element.getName())){
						wXPayResult.setTotal_fee(element.getText());
					}
