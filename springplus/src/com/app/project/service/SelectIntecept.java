	...
	public void selectEnd(String sqlId,Map<String, String> parameter,Result result) {
		...
		if (sqlId.equals("plancustomlist")) {
			...
			for (Map<String, Object> map : listResult) {
				...
				if (!idtype.equals("plan")) {
					List<Map<String, Object>> listMaps = baseDao.getListMaps("select visitDate,visitProject,id as tripId from usertrip where visitCustomId='"+map.get("customid")+"' and state=1 and left(visittime,7)='"+map.get("tripdate")+"' order by visitTime desc limit 0,3");
